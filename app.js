...
async function generatePdf() {
  ...
  // TIN box centers, recalibrated so digits shoot into the actual TIN boxes
  // in 3, 3, 3, 5 format, just like the date fields are split by box.
  const tinCenters = [
    214.0, 227.5, 241.0,
    268.0, 281.5, 295.0,
    322.0, 335.5, 349.0,
  ...
  const zipCenters = [552.0, 565.5, 579.0, 592.5];
  ...
  // Part I - Payee TIN, properly split into boxes
  drawDigits(page, tinDigits, tinCenters, 785.8, 9.8, { font: boldFont, color: black });
  ...
  if (currentPreview.zip) {
    drawDigits(page, onlyDigits(currentPreview.zip).padStart(4, "0").slice(-4), zipCenters, 715.0, 9.0, { font: boldFont, color: black });
  }

  // Part II - Payor information, properly split into boxes
  page.setFont(boldFont);
  drawDigits(page, payorTinDigits, tinCenters, 657.8, 9.8, { font: boldFont, color: black });
  ...
  drawDigits(page, "1870", zipCenters, 607.0, 9.0, { font: boldFont, color: black });
