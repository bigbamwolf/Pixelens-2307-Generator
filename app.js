...
async function generatePdf() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Cannot load ${TEMPLATE_PDF}`);
    }
  ...
  // Digit centers calibrated for the uploaded original 2307 PDF.
  // These draw each number inside its own BIR box instead of printing grouped numbers with slashes/hyphens.
  ...
  const tinCenters = [
    ...
    158.8, 172.3, 185.8,
    210.5, 224.3, 237.8,
    259.3, 274.5, 290.8, 306.5, 322.8
  ];

  const payorTinCenters = tinCenters;
  const payeeZipCenters = [547.5, 561.8, 575.3, 588.5];
  const payorZipCenters = payeeZipCenters;

  page.setFont(boldFont);

  // Part I - Period, digit-by-digit.
  ...
  // Part I - Payee TIN, digit-by-digit.
  drawDigits(page, tinDigits, tinCenters, 786.7, 10, { font: boldFont, color: black });

  // Part I - Payee name and address.
  page.setFont(font);
  drawText(page, supplier.name, 68, 758, 10.5, { color: black, maxWidth: 500, lineHeight: 11.5 });
  drawText(page, supplier.address, 68, 712, 8.8, { color: black, maxWidth: 492, lineHeight: 9.6 });

  if (currentPreview.zip) {
    drawDigits(page, onlyDigits(currentPreview.zip).padStart(4, "0").slice(-4), payeeZipCenters, 715, 9.2, { font: boldFont, color: black });
  }

  // Part II - Payor information, digit-by-digit.
  page.setFont(boldFont);
  drawDigits(page, payorTinDigits, payorTinCenters, 660.2, 10, { font: boldFont, color: black });

  page.setFont(font);
  drawText(page, "PIXELENS CREATIVE ADVERTISING INC.", 68, 632.5, 10, { color: black, maxWidth: 500 });
  drawText(page, "G8-4 2ND FLOOR GEMS PLAZA CIRCUMFERENTIAL ROAD SAN JOSE, ANTIPOLO CITY", 68, 604.5, 8.2, { color: black, maxWidth: 492, lineHeight: 9 });
  drawDigits(page, "1870", payorZipCenters, 606, 9.2, { font: boldFont, color: black });

  // Part III - Income details.
  ...
  // Totals row, right-aligned inside proper boxes.
